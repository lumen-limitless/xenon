...
export default async function Page({}: PageProps) {
  ...
  const lastWeekProfit = orders
    .filter(
      (order) =>
        order.createdAt > new Date(Date.now() - 14 * 24 * 60 * 60 * 1000) &&
        order.createdAt < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
    )
    .reduce((acc, order) => acc + order.total, 0);
  ...
  const lastMonthProfit = orders
    .filter(
      (order) =>
        order.createdAt > new Date(Date.now() - 60 * 24 * 60 * 60 * 1000) &&
        order.createdAt < new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
    )
    .reduce((acc, order) => acc + order.total, 0);
